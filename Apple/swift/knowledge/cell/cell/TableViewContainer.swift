...
// UIViewControllerRepresentable：将 UIKit 的 ViewController 包装为 SwiftUI 视图
struct TableViewContainer: UIViewControllerRepresentable {
    ...
    // 更新已存在的ViewController
    // 状态变化时候被调用（如@State、@Binding）
...
class TableViewController: UIViewController {
    ...
    private func generateData() {
        let icons = [
            "star.fill", "heart.fill", "book.fill", "music.note",
...
extension TableViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // 注意：如果是新创建的 cell，会在 init 中打印日志
        // 如果是复用的 cell，会在 prepareForReuse 中打印日志
        let cell =
        ...
        if indexPath.row < 20 || indexPath.row % 100 == 0 {
            print("📱 配置 Cell for row \(indexPath.row) - 标题: \(item.title)")
        }
